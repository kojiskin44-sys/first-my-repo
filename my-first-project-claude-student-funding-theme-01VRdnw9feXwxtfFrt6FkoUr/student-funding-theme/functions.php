...
/**
 * デフォルトのプロジェクトカテゴリーを作成
 */
function student_funding_create_default_categories() {
    // テーマ有効化時に一度だけ実行
    if (get_option('student_funding_categories_created')) {
        return;
    }
    ...
    foreach ($default_categories as $category_name) {
        // カテゴリーが存在しない場合のみ作成
        if (!term_exists($category_name, 'project_category')) {
            wp_insert_term($category_name, 'project_category');
        }
    }

    // フラグを設定して再実行を防ぐ
    update_option('student_funding_categories_created', true);
}
add_action('after_switch_theme', 'student_funding_create_default_categories');
